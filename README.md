...
Agent-Home 是一个面向 Agent 的本地状态底座，用 REST daemon 的形式为 Agent 提供会话时间线、工作区目录和长期记忆的统一管理能力。
...
Agent-Home 将这些能力收敛到一个本地优先的服务层中。Agent Core 只需要调用 API，不需要直接关心底层如何存储 session、message、workspace 或 memory。
...
- 基于 `.workspace/<agent_id>` 的本地 workspace 目录；
- 基于 Agent token 的接口鉴权；
- timeline、workspace、memory 三类核心状态能力。

默认服务监听 `127.0.0.1:8765`，默认数据库文件为 `agent_home.sqlite`，Agent workspace 默认创建在 `.workspace/<agent_id>/` 下。
...
Workspace 模块用于为每个 Agent 管理一个独立的本地工作区目录。创建 Agent 时，Agent-Home 会立即创建：

```text
.workspace/<agent_id>/
```

后续 workspace command 会直接在该目录中执行。Agent 可以通过命令创建文件、读取文件、写入文件、删除文件和运行终端命令，行为与操作普通本地 workspace 一致。

已实现能力：

- 创建 Agent 时创建对应 workspace 目录；
- 在 Agent workspace 目录中执行命令；
- 返回命令的 exit code、stdout 和 stderr；
- 按 Agent 隔离 workspace 目录；
- 同一个 Agent 的 workspace 命令串行执行。

当前 workspace 是本地优先实现：每个 Agent 的工作区是 `.workspace/<agent_id>/` 下的真实目录。`agent_id` 必须是一串数字，允许前导零。命令执行不提供额外文件系统沙箱；如果命令访问绝对路径，行为与本地 shell 一致。
...
Agent-Home REST API
  ...
  └─ Memory Service
        ...
        └─ Local agent workspace directories
```
...
| `agent_home/workspace.py` | workspace command API，在 Agent 本地 workspace 目录中执行命令 |
| `agent_home/workspace_manager.py` | workspace 路径校验、目录计算和目录创建 |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/v1/agents/{agent_id}/workspace/commands` | 在 Agent workspace 目录中执行命令 |
