...
def test_list_and_patch_sessions(tmp_path: Path):
    ...
    session = client.post("/v1/agents/123/sessions", headers=headers, json={"session_id": "s1", "title": "old"}).json()

    listed = client.get("/v1/agents/123/sessions", headers=headers)
    patched = client.patch(
        "/v1/agents/123/sessions/s1",
        headers=headers,
...
def test_patch_session_preserves_omitted_fields(tmp_path: Path):
    client, headers = make_client(tmp_path)
    session = client.post(
        "/v1/agents/123/sessions",
        headers=headers,
        json={"session_id": "s1", "title": "old", "metadata": {"k": "v"}},
    ).json()
    active_branch_id = session["active_branch_id"]

    title_patch = client.patch("/v1/agents/123/sessions/s1", headers=headers, json={"title": "new"})
    ...
    metadata_patch = client.patch("/v1/agents/123/sessions/s1", headers=headers, json={"metadata": {"next": True}})
...
def test_patch_session_rejects_branch_from_another_session(tmp_path: Path):
    client, headers = make_client(tmp_path)
    client.post("/v1/agents/123/sessions", headers=headers, json={"session_id": "s1"})
    other = client.post("/v1/agents/123/sessions", headers=headers, json={"session_id": "s2"}).json()
...
def test_branch_create_and_get(tmp_path: Path):
    client, headers = make_client(tmp_path)
    session = client.post("/v1/agents/123/sessions", headers=headers, json={"session_id": "s1"}).json()

    created = client.post(
        "/v1/agents/123/sessions/s1/branches",
        headers=headers,
        json={
            "branch_id": "b-custom",
    ...
    loaded = client.get("/v1/agents/123/branches/b-custom", headers=headers)
...
def test_create_branch_under_missing_session_returns_session_not_found(tmp_path: Path):
    ...
    created = client.post(
        "/v1/agents/123/sessions/missing/branches",
        headers=headers,
...
def test_get_run_latest_run_and_checkpoint(tmp_path: Path):
    ...
    client.post("/v1/agents/123/runs", headers=headers, json={"run_id": "r1", "session_id": "s1", "branch_id": branch_id})
    client.post(
        "/v1/agents/123/messages",
        headers=headers,
        json={
            "message_id": "m1",
            "session_id": "s1",
    ...
    run = client.get("/v1/agents/123/runs/r1", headers=headers)
    latest = client.get(f"/v1/agents/123/branches/{branch_id}/runs/latest", headers=headers)
    checkpoint = client.get("/v1/agents/123/checkpoints/c1", headers=headers)
...
def test_latest_run_on_empty_branch_returns_run_not_found(tmp_path: Path):
    ...
    latest = client.get(f"/v1/agents/123/branches/{session['active_branch_id']}/runs/latest", headers=headers)
...
def test_missing_compatibility_endpoints_return_structured_errors(tmp_path: Path):
    ...
    branch = client.get("/v1/agents/123/branches/missing", headers=headers)
    run = client.get("/v1/agents/123/runs/missing", headers=headers)
    checkpoint = client.get("/v1/agents/123/checkpoints/missing", headers=headers)
